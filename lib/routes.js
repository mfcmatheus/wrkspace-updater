...
module.exports = ({ cache, config }) => {
    ...
    if (compare(latest.version, version) !== 0) {
      ...
      send(res, 200, {
        ...
        url: shouldProxyPrivateDownload
          ? `https://updater.wrkspace.co/download/${platformName}?update=true`
  ...
  exports.updateGetFile = async (req, res) => {
    const { platform: platformName, version, file } = req.params
    ...
    if (compare(latest.version, version) !== 0) {
      if (shouldProxyPrivateDownload) {
        ...
        const releaseUrl = `https://api.github.com/repos/${account}/${repository}/releases/tags/${
          latest.version
        }`
        ...
        if (!releaseResponse.ok) {
          send(res, 500, 'Error fetching release data from GitHub')
          return
        }

        const releaseData = await releaseResponse.json()

        const asset = releaseData.assets.find(a => a.name === file)
        ...
        const assetUrl = asset.url // API URL to download the asset
        ...
        const assetResponse = await fetch(assetUrl, { headers: assetHeaders })

        if (!assetResponse.ok) {
          send(res, 500, 'Error downloading asset from GitHub')
          return
        }

        // Set headers and stream the response to the client
        res.setHeader('Content-Disposition', `attachment; filename="${file}"`)
        res.setHeader('Content-Type', assetResponse.headers.get('Content-Type'))
        res.setHeader(
          'Content-Length',
      ...
      return res.redirect(
        302,
        `https://github.com/${account}/${repository}/releases/download/${
          latest.version
        }/${file}`
      )
    }
