...
module.exports = ({ cache, config }) => {
  ...
  exports.getLatestMac = async (req, res) => {
    const { platform: platformName, version } = req.params
    const latest = await loadCache()

    if (!latest.platforms || !latest.platforms.darwin) {
      res.statusCode = 204
      res.end()
    ...
    res.redirect(302, `https://github.com/${account}/${repository}/releases/download/${version}/latest-mac.yml`)
  }
